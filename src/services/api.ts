...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        const refreshToken = await AsyncStorage.getItem('@simply:refreshToken');
        if (refreshToken) {
          ...
          await AsyncStorage.setItem('@simply:refreshToken', newRefreshToken);
        ...
        // Emitir evento de logout (se manejará en el store)
      }
    }
